...
- for causal relations (e.g. "because", "caused", "led to", "due to"), use a `Cause` predicate with type definition `(: Cause (-> Concept Concept Type))` relating two event instances, e.g. "The rain caused the flood" → `(And (RainEvent rain_1) (FloodEvent flood_1) (Cause rain_1 flood_1))`; do NOT fold causality into a plain `Implication` — causality is a specific real-world mechanism between particular event instances, whereas `Implication` expresses a generic statistical rule between predicates
