...
- no instance can exist on its own within the `expr_body`, each instance needs to be associated with a predicate, and so in this representation essentially every linguistic term (e.g. nouns, pronouns, verbs, adjectives, adverbs, or even prepositions etc.) is captured by associating an instance (or variable) with a predicate, arranged and grouped together with the built-in logical operators or other predicates, constituting the `expr_body`
- similarly no predicate can exist on its own without associating with an instance or a variable, so it should look like: `(<predicate> <instance or variable>)`, which covers both categorical membership (e.g. `(Dog fido)` for "Fido is a dog") and property/attribute attribution (e.g. `(Fast fido)` for "Fido is fast") — in both cases the `predicate` denotes the category or property that the instance belongs to or has
...
- if a multi-word predicate is really necessary to be created, you must also unpack and elaborate its meaning by creating additional `Implication` expression(s); how to do this depends on the semantics of the compound:
  - if each part of the predicate is a genuine co-property of the same instance (e.g. "PlayingCoach" = one who both plays and coaches), use the conjunction-to-compound direction: `(Implication (PlayingCoach $a) (And (Player $a) (Coach $a)))`
  - if the compound is a modifier + head noun (e.g. "RedDoor" = a door that is red), express it as a subtype of the head noun with a separate predicate capturing what the modifier contributes: `(Implication (RedDoor $x) (And (Red $x) (Door $x)))`; critically, do NOT apply the raw modifier word as a predicate to the instance in cases where the modifier denotes a related concept rather than a direct property — e.g. for a compound like "SteamEngine", `(Steam $x)` would wrongly assert the engine itself IS steam, when the correct reading is that it is powered by steam, so the right unpacking would be `(Implication (SteamEngine $x) (And (Steam $y) (Engine $x)(PoweredBy $x $y)))`
...
- typically `Implication` is used to express generic/categorical "IsA" or if-then relationships — i.e. when a statement applies to a category as a whole using variables (e.g. "A cat is a mammal" → `(Implication (Cat $x) (Mammal $x))` meaning all cats are mammals); by contrast, instance-level "is-a" (e.g. "Fido is a dog") and "has-property-of" (e.g. "Fido is fast") both use the simple predicate-application form — `(Dog fido)` and `(Fast fido)` respectively — not `Implication`
