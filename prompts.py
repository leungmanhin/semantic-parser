...
- for PLN expressions that are not type definitions, they should be in the format of `(: <expr_name> <expr_body> <truth_value>)`, with the `truth_value` typically provided using the built-in constructor `STV`, which expects a strength and a confidence value; for expressions involving numeric measurements, a distribution truth value (e.g. `PointMass`, `ParticleFromNormal`) can be used instead — see the section on value uncertainty below
...
- to denote a predicate (i.e. a verb or multi-argument relation — not nouns or adjectives, which are handled as concept names via `IsA`, `HasAttribute`, and `Has`), it's preferable to name it using the original word or phrase in the given sentence without adding any pre- or post-fix, and it must be written in UpperCamelCase; predicate names must always use the base (infinitive) form of the verb or phrase — never inflected for tense, number, or person (e.g., `Form` not `Forms`, `DependOn` not `DependsOn`, `Give` not `Gave`); all tense and aspect information is captured separately via `TemporalBefore`/`TemporalAfter`/`TemporalOverlap` etc. and event reification
...
- no instance can exist on its own within the `expr_body`, each instance needs to appear within a predicate application; how each linguistic term is captured depends on its role:
  ...
  - verbs and multi-argument relations become UpperCamelCase predicates applied to their arguments, e.g. "Alice loves Bob" → `(Love alice_1 bob_1)`
...
- similarly no predicate can exist on its own without associating with an instance or a variable; there are three built-in predicates for the most common instance-to-concept relations (one instance plus one generic concept), and you should use them instead of bare custom predicates wherever they apply:
  ...
  - for relations between two or more instances (e.g. "Alice loves Bob", "Bob gave Alice a book"), use a custom predicate in UpperCamelCase as before: `(Love alice_1 bob_1)`, `(Give bob_1 alice_1 book_1)`
...
- when using `IsA` with a compound concept name (e.g. `steam_engine`, `red_door`), add an `Implication` unpacking what that concept entails so the reasoner can use it; unpack into the head category plus whatever the modifier contributes — using `HasAttribute` if the modifier is a direct intrinsic property of the instance, or a proper binary relation if the modifier denotes a related concept: e.g. `(Implication (Premises (IsA $x red_door)) (Conclusions (IsA $x door) (HasAttribute $x red)))` is correct since red is a direct property of the door, but for `steam_engine` the engine is not itself "steam" — so express the actual relation introducing a fresh Conclusions-only variable `$y` for the steam instance (existentially quantified per the variable scoping rule — asserting that some steam entity exists and powers the engine): `(Implication (Premises (IsA $x steam_engine)) (Conclusions (IsA $x engine) (IsA $y steam) (PoweredBy $x $y)))`
...
- `Implication` must always use the explicit `Premises`/`Conclusions` wrapper syntax: `(Implication (Premises <premise1> <premise2> ...) (Conclusions <conclusion1> ...))` — this is required by the chainer's compiler; when there are multiple premises, list them directly inside `Premises` — they are implicitly conjoined (no need to wrap them in `And`); similarly, multiple conclusions can be listed directly inside `Conclusions` and are also implicitly conjoined; `And`, `Or`, and `LikelierThan` can still be used explicitly within a single premise or conclusion slot for compound sub-expressions
- typically `Implication` is used to express generic/categorical "IsA" or if-then relationships — i.e. when a statement applies to a category as a whole using variables (e.g. "A cat is a mammal" → `(Implication (Premises (IsA $x cat)) (Conclusions (IsA $x mammal)))` meaning all cats are mammals); by contrast, instance-level "is-a" (e.g. "Fido is a dog"), "has-property-of" (e.g. "Fido is fast"), and "has-condition" (e.g. "Bob has cancer") all use `IsA`, `HasAttribute`, and `Has` directly — `(IsA fido_1 dog)`, `(HasAttribute fido_1 fast)`, and `(Has bob_1 cancer)` respectively — not `Implication`
- variables that appear in the `Premises` of an `Implication` are universally quantified and scoped within that `Implication` (and everything nested within it); variables that appear only in the `Conclusions` are existentially quantified — they assert that some entity satisfying that description exists in the consequent (e.g. in `(Implication (Premises (IsA $x author)) (Conclusions (IsA $y book) (Wrote $x $y)))`, `$x` is universally quantified over authors while `$y` asserts the existence of a book that each such author wrote)
...
- if applying a modal to a multi-arity predicate, wrap the entire expression: `(Could (Cut bob_1 apple_1))`; if applying a modal to a Neo-Davidsonian event, apply it to the entire event structure: `(Could (And (IsA cut_evt_1 cut_event) (Agent cut_evt_1 bob_1) (Theme cut_evt_1 apple_1)))`
...
- for comparatives (e.g. "Alice is taller than Bob"), create a binary predicate for the comparison, e.g. `(: alice_taller_than_bob (TallerThan alice_1 bob_1) (STV 0.95 0.9))` with type definition `(: TallerThan (-> Concept Concept Type))`; and add a bridging `Implication` connecting it to the base predicate where meaningful, e.g. `(: implication_taller_than_tall (Implication (Premises (TallerThan $x $y)) (Conclusions (HasAttribute $x tall))) (STV 0.6 0.8))` — the strength is intentionally modest since being taller than a specific individual does not guarantee being tall in an absolute sense, but you should use this as a reference only and make your judgement based on the actual context and sentence given
- for superlatives (e.g. "Alice is the tallest runner"), represent it as a universally quantified statement meaning no other member of the category exceeds the subject, e.g. `(: alice_tallest_runner (Implication (Premises (IsA $x runner) (Not (Similarity $x alice_1))) (Conclusions (TallerThan alice_1 $x))) (STV 1.0 0.9))`
...
- `(<Predicate> <instance_1> <instance_2> ...)` for verbs and relations — e.g. `(: alice_loves_bob (Love alice_1 bob_1) (STV 0.9 0.9))`
...
- **Or**: `strength_result = min(s1, s2, ...)`, `confidence_result = max(c1, c2, ...)`; the disjunction's strength is the minimum of its members and its confidence is the maximum
...
The system distinguishes between truth uncertainty and value uncertainty:
- **Truth uncertainty** (whether a proposition is true) is represented with `STV` as described above
- **Value uncertainty** (what a numeric value is) is represented with distribution truth values; these replace the `STV` slot in a PLN expression and are useful when a sentence involves specific numeric measurements or uncertain quantities; the available distribution constructors are:
  - `(PointMass <value>)` — an exact numeric value, e.g. "Alice is 160cm tall" → type definition `(: Height (-> Concept Type))` and `(: alice_height (Height alice_1) (PointMass 160.0))`
  - `(ParticleFromNormal <mean> <stddev>)` — an uncertain numeric value modeled as a normal distribution, e.g. "Alice is about 160cm tall" → type definition `(: Height (-> Concept Type))` and `(: alice_height (Height alice_1) (ParticleFromNormal 160.0 5.0))`
  - `(ParticleFromPairs ((<value1> <weight1>) (<value2> <weight2>) ...))` — a discrete distribution over specific values, e.g. "the coin is either 5g or 10g" → type definition `(: Weight (-> Concept Type))` and `(: coin_weight (Weight coin_1) (ParticleFromPairs ((5.0 0.5) (10.0 0.5))))`
  - `(NatDist ((<value> <probability>) ...))` and `(FloatDist ((<value> <probability>) ...))` — exact discrete distributions over natural numbers and floats respectively
- when using distribution TVs, keep membership/existence truth in separate `STV` facts if needed, e.g. `(: alice_is_person (IsA alice_1 person) (STV 1.0 1.0))` alongside `(: alice_height (Height alice_1) (PointMass 160.0))`; do not encode numeric values in `STV` strength for measurement semantics
- use `HasAttribute` with `STV` for qualitative properties that describe an instance on a subjective or categorical scale (e.g. "Alice is tall" → `(HasAttribute alice_1 tall)`); use a custom measurement predicate with a distribution TV for quantitative properties that have a specific numeric value or range (e.g. "Alice is 160cm tall" → `(Height alice_1)` with `(PointMass 160.0)`); if a sentence provides both (e.g. "Alice is tall at 180cm"), represent both: the qualitative `HasAttribute` with an appropriate STV strength reflecting the degree, and the quantitative measurement predicate with the numeric value

The system also provides computational premise helpers that can be used inside the `Premises` of an `Implication` rule:
- `(Compute <function> (<arg1> <arg2> ...) -> <$output>)` — runs a function and binds the output to a variable, e.g. `(Compute + ($a $b) -> $sum)` or `(Compute NotEqual ($name MissingTotal) -> True)`; useful for rules that need arithmetic or comparison operations
- `(GreaterThan <dist_fact> <threshold>)` — tests whether a distribution-valued fact exceeds a numeric threshold, e.g. `(GreaterThan (CountryHeightDist countryA) 170)`; can also compare two distribution-valued facts: `(GreaterThan (CountryHeightDist countryA) (CountryHeightDist countryB))`
- `(MapDist <function> <dist_fact> -> <$output>)` — applies a unary function to a distribution-valued fact, e.g. `(MapDist (|-> ($x) (+ $x 1)) (HeightDist $person) -> $adjustedDist)`
- `(Map2Dist <function> <dist_fact_a> <dist_fact_b> -> <$output>)` — applies a binary function to two distribution-valued facts, e.g. `(Map2Dist * (LengthDist $rect) (WidthDist $rect) -> $areaDist)` for computing the area distribution from length and width
- `(AverageDist <pattern> -> <$output>)` — computes the average distribution over all matching facts, e.g. `(AverageDist (HeightDist $group $person) -> $avgDist)`
- `(FoldAll <pattern> <value> <init> <fold_fn> -> <$output>)` — aggregates plain values over all matching facts, e.g. `(FoldAll (Score $name $n) $n 0 (|-> ($acc $elem) (+ $acc $elem)) -> $total)` to sum all scores
- `(FoldAllValue <pattern> <value> <init> <fold_fn> -> <$output>)` — like `FoldAll` but aggregates over truth values (distributions), e.g. `(FoldAllValue (In $room $kid) $tvin (ParticleFromPairs ((0 1.0))) ParticleAddBernoulliFromSTV -> $dist)` to combine Bernoulli distributions from STV-valued facts
