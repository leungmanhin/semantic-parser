...
- to denote an instance, name it after the core noun or phrase it refers to, written in snake_case, stripping away determiners like "the", "a", "an" and using the singular base form regardless of grammatical number, and always append a numeric suffix `_1`, `_2`, etc. to distinguish specific instances from generic concept names (e.g. "the cat" → `cat_1`, "Alice" → `alice_1`, "a rain event" → `rain_evt_1`); for event instances specifically, use the pattern `<name>_evt_1`, `<name>_evt_2`, etc. (e.g. "a cutting event" → `cut_evt_1`); for group instances, use `<name>_group_1` (e.g. "the cats" → `cat_group_1`); plurality or quantity is captured separately via the `Quantity` predicate or the group/collective pattern (e.g. "3 cats" → `(And (IsA cat_group_1 cat) (Quantity cat_group_1 3))`), not by pluralising the instance name or predicate; if the same entity is referred to more than once within the same sentence, reuse the exact same instance name rather than creating a new one; only use `_2`, `_3`, etc. when two or more distinct entities in the same sentence would otherwise share the same base name (e.g. "Alice met another Alice" → `alice_1` and `alice_2`); global uniqueness across sentences is handled by post-processing, so you do not need to worry about collisions with other sentences
...
- to denote a predicate (i.e. a verb or multi-argument relation — not nouns or adjectives, which are now handled as concept names via `IsA` and `HasAttribute`), it's preferable to name it using the original word or phrase in the given sentence without adding any pre- or post-fix, and it must be written in UpperCamelCase
...
- no instance can exist on its own within the `expr_body`, each instance needs to appear within a predicate application; how each linguistic term is captured depends on its role:
  ...
  - adjectives and adverbs denoting intrinsic qualities appear as the second argument of `HasAttribute`, e.g. "Fido is fast" → `(HasAttribute fido_1 fast)`, where `fast` is a concept name, not a predicate
...
- similarly no predicate can exist on its own without associating with an instance or a variable; there are two built-in predicates for the most common unary relations, and you should use them instead of bare custom predicates wherever they apply:
  ...
  - do NOT create bare custom unary predicates like `(Dog fido_1)` or `(Fast fido_1)` — use `IsA` and `HasAttribute` instead
...
- with `IsA` and `HasAttribute`, multi-word predicates now arise mainly for verbs and verb phrases (e.g. `LivesIn`, `BelongTo`), since nouns are handled via `IsA` and adjectives via `HasAttribute`; if a multi-word verb predicate is necessary, unpack its meaning with an `Implication` in terms of simpler predicates
- `HasAttribute` is only appropriate for intrinsic scalar or qualitative properties that directly describe the instance itself (e.g. color, size, shape, temperature, speed) — e.g. `(HasAttribute door red)` for "the door is red", `(HasAttribute alice tall)` for "Alice is tall"; do NOT use it in these cases:
  - when the quality belongs to a related action rather than the instance itself — e.g. in "she walks fast", "fast" modifies the walking action, not the person, so `(HasAttribute alice fast)` would be wrong; instead reify the event and attribute the quality to it: `(And (IsA walk_evt_1 walk_event) (Subject walk_evt_1 alice_1) (HasAttribute walk_evt_1 fast))`
  ...
  - when the "attribute" is a condition or entity the instance possesses rather than a quality that describes it — use a binary relation predicate instead, e.g. "Bob has cancer" → `(Has bob_1 cancer)`, not `(HasAttribute bob_1 cancer)`
- when using `IsA` with a compound concept name (e.g. `steam_engine`, `red_door`), add an `Implication` unpacking what that concept entails so the reasoner can use it; unpack into the head category plus whatever the modifier contributes — using `HasAttribute` if the modifier is a direct intrinsic property of the instance, or a proper binary relation if the modifier denotes a related concept: e.g. `(Implication (IsA $x red_door) (And (IsA $x door) (HasAttribute $x red)))` is correct since red is a direct property of the door, but for `steam_engine` the engine is not itself "steam" — so express the actual relation introducing a fresh variable for the steam instance: `(Implication (IsA $x steam_engine) (And (IsA $x engine) (IsA $y steam) (PoweredBy $x $y)))`
- you must create a type definition for every new predicate that you are creating and using; note that concept names used as the second argument of `IsA` or `HasAttribute` (e.g. `dog`, `fast`, `runner`) are not predicates and do not need type definitions — only UpperCamelCase predicates (verbs, relations, and built-in operators) require them
...
- when a sentence contains an indefinite noun phrase (e.g. "a cat", "an idea", "some book") that refers to a particular entity in context — even if its identity is unknown or unnamed — create a fresh instance for it (e.g. "A cat sat on the mat" → create instance `cat`); contrast this with a generic or categorical use of an indefinite NP that makes a claim about the whole category (e.g. "A cat is a mammal" = "All cats are mammals"), which should be represented as a generic `Implication` with a variable rather than a fresh instance; the key test: does the sentence report a fact about a specific individual (even unnamed), or make a general statement about the category as a whole?
